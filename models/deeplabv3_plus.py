...
def build_torchvision_backbone(backbone: str, pretrained: bool):
    # torchvision >=0.13 推荐用 weights
    if pretrained:
        return getattr(models, backbone)(weights="DEFAULT")
    else:
        return getattr(models, backbone)(weights=None)
...
class ResNet(nn.Module):
    def __init__(self, backbone, in_channels=3, output_stride=16, pretrained=True):
        super().__init__()
        model = build_torchvision_backbone(backbone, pretrained)
        if not pretrained or in_channels != 3:
            self.layer0 = nn.Sequential(
                nn.Conv2d(in_channels, 64, 7, stride=2, padding=3, bias=False),
                # 卷积层：输入通道数，输出通道数，卷积核大小，步长，填充，是否启用偏置项
                nn.BatchNorm2d(64),
                # 批量归一化层：64个输出通道
                nn.ReLU(inplace=False),
                # RELU激活函数：inplace=True: 这意味着将在原地进行操作，节省内存消耗，但会改变输入数据。
                nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
                # 最大池化层：池化窗口大小，步长，边缘填充
            )
            initialize_weights(self.layer0)
            # initialize_weights(self.layer0) 函数可能被用来初始化这一层的权重。
            # 这是一个常见的做法，可以帮助避免训练开始时的梯度问题，使模型的训练更加稳定和高效。具体的初始化方式取决于 initialize_weights 函数的实现细节。
        else:
...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, dilation=1, exit_flow=False, use_1st_relu=False):
        super().__init__()
...
class DeepLab(BaseModel):
    def __init__(self, num_classes, backbone, in_channels=3, pretrained=False,
        ...
        self.ASSP = ASSP(in_channels=aspp_in_ch, output_stride=output_stride)
